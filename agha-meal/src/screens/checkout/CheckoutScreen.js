...
const CheckoutScreen = ({ navigation, route }) => {
  ...
  const calculateTotal = () => {
    let total = 0
    total = Number.parseFloat(calculateSubtotal()) + Number.parseFloat(deliveryCost || 0)
    return total
  }
    ...
    <SafeAreaView style={Style.container}>
      ...
      <ScrollView style={Style.listContainer} showsVerticalScrollIndicator={false}>
        ...
        {Object.entries(ordersByName).map(([itemName, order]) => {
          return <CheckoutItem key={itemName} name={itemName} order={order.orders} />
        })}
      </ScrollView>
      ...
      <View style={Style.summaryCard}>
        ...
        <View style={Style.detailsContainer}>
          ...
          <View style={Style.detailsRowContainer}>
            ...
            <Text style={Style.detailValue}>{deliveryCost ? deliveryCost : 0} JOD</Text>
          </View>
